...
def findall_list():
    try:
        ...
        # SQL 실행
        sql = "select b.no, b.title, u.name, b.hit, b.reg_date, b.depth from board b, user u where b.user_no = u.no order by b.g_no desc, b.o_no"
        cursor.execute(sql)
...
def update(title, contents, no):
    try:
        ...
        count = cursor.execute(sql, (title, contents, no))   # 바인딩
...
def search(kwd):
    try:
        ...
        # SQL 실행
        sql = '''
              select temp.no, temp.title, temp.name, temp.hit, temp.reg_date
              from (select b.no, b.title, b.contents, u.name, b.hit, b.reg_date from board b, user u where b.user_no = u.no) as temp
              where temp.name like %s
              or temp.title like %s
              or temp.contents like %s
