...
```javascript
!Number.isNaN(0.123);         // true
!Number.isNaN(123 / "hello"); // true
Number.isNaN(123 / "hello");  // true
...
- `String.slice(0,10)` : start보다 end가 큰 경우에는 빈 문자열을 반환한다 
- `String.substring(0,10)` : start보다 end가 큰 경우, 내부적인 로직에 의해 작은 지점부터 큰 지점까지의 문자열을 추출해 반환한다
```javascript
let text = "hello, world!!!";
